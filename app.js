...
function tickClock() {
  ...
  if (!clockEl) return;
  const elapsed = Date.now() - new Date(currentStatus.since).getTime();
...
async function loadData() {
  try {
    const statusRes = await fetch(`${STATUS_URL}?t=${Date.now()}`, { cache: "no-store" });
  ...
  try {
    const profileRes = await fetch(`${PROFILE_URL}?t=${Date.now()}`, { cache: "no-store" });
  ...
  try {
    const historyRes = await fetch(`${HISTORY_URL}?t=${Date.now()}`, { cache: "no-store" });
